...
num_files = 10  # Number of log files to generate
...
date_end = "2023-5-01"  # End date for log files
...
# External IPs
for i in range(0, int(min_num_lines * num_files / 50 + 1)):
    ip_list.append(str(random.randint(1, 254)) + "." + str(random.randint(1, 254)) +
...
user_agents = random.choices(
    list(ua_pct['ua'].values()), list(ua_pct['pct'].values()), k=100)
...
for i in range(0, num_files):
    ...
    for j in range(0, random.randint(min_num_lines, max_num_lines)):
        ...
        # Create status code
        status_code = str(random.choices(status_codes, status_codes_distribution)[0])
